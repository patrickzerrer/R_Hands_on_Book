...
![Grafik einfacher linearer Zusammenhang zwischen Fernsehnutzung und Anzahl der rezipierten Werbespots](Bilder/Einfaches Beispiel linearer Zusammenhang SG.png)
...
## Das Überprüfen von Zusammenhängen bei zwei nominalskalierten (bis ordinalskalierten) Variablen
...
## Erzeugen eines Teildatensatzes, Umbenennen und Filtern der Variablen
```{r}
daten <- daten %>%
...
## Visualisieren des Zusammenhangs mit Hilfe eines gestapelten Balkendiagramms 
...
## Überprüfen des Zusammenhangs mit dem Chi-Quadrat Test
...
- Bei *Spearman's Rho* (auch als *Rangkorrelation nach Pearson* bezeichnet) werden die Daten in Ränge umgewandelt, und dann wird der Pearson-Korrelationskoeffizient auf den Rängen berechnet. Spearman's Rho ist besonders robust gegenüber Ausreißern (auch etwas mehr als Kendall's Tau). Spearman's Rho  berücksichtigt die genauen Abstände zwischen den Rängen und kann subtilere Beziehungen zwischen den Variablen erkennen.
...
# Beispiel: Die Analyse von Zusammenhängen zwischen zwei ordinalskalierten Variablen
...
# Das Überprüfen von Zusammenhängen zwischen einer nominalen und einer ordinalen Variable 
