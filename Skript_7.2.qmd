...
Die Korrelationsanalyse untersucht den Zusammenhang zwischen zwei metrischen Variablen. Als statistischer Kennwert wird ein *Korrelationskoeffizient* ermittelt. Ein positiver Korrelationskoeffizient deutet auf einen positiven Zusammenhang hin, während ein negativer Koeffizient auf einen negativen Zusammenhang hindeutet.

# Die Analyse von Zusammenhängen bei zwei metrischen Variablen mit Hilfe der Korrelationsanalyse
Die Korrelationsanalyse ist eine schnelle und einfache statistische Methode, um den Zusammenhang zwischen zwei oder mehr metrischen Variablen zu untersuchen. Ziel der Korrelationsanalyse ist, die Richtung (positiv oder negativ) und die Stärke eines linearen Zusammenhangs zwischen den Variablen zu bestimmen. Sie ist allerdings etwas empfindlich gegenüber Ausreißern und setzt einen linearen Zusammenhang voraus.

Wenn ein linearer Zusammenhang vermutet wird, kann der *Pearson-Korrelationskoeffizient* zur Analyse genutzt werden. Pearson's r ist ein Maß für den linearen Zusammenhang zwischen zwei metrischen Variablen. Er misst die Stärke und Richtung des Zusammenhangs und nimmt Werte zwischen -1 und 1 an. Ein hoher Korrelationskoeffizient (nahe 1 oder -1) deutet auf einen starken Zusammenhang hin, während ein niedriger Koeffizient (nahe 0) auf einen schwachen oder keinen Zusammenhang hindeutet.
...
Die einfache lineare Regression analysiert den Zusammenhang zwischen einer abhängigen metrischen Variable und einer unabhängigen metrischen Variable. Sie gibt an, wie sich die abhängige Variable ändert, wenn sich die unabhängige Variable um eine Einheit ändert.
...
In diesem Kapitel wird die **einfache lineare Regression** auf Grundlage der ESS-Daten vorgestellt. In der nächsten Sitzung gehen wir näher auf die Prüfung der **Voraussetzungen einer Regressionsanalyse** ein und lernen die **multiple lineare Regression** kennen.
