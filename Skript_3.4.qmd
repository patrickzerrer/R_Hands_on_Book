...
ggplot(sample_klein, aes(as_factor(fernsehkonsum))) + 
  ...
  ggtitle("Fernsehkonsum pro Woche") + 
...
ggplot(sample_gross, aes(vertrauen_bundestag, fill = geschlecht)) + 
  geom_histogram(binwidth = 0.5, position = "dodge") + 
  ggtitle("Vertrauen in den Bundestag nach Geschlecht") + 
  xlab("Vetrauen (0-10)") + ylab("Anzahl der Respondenten") + labs(fill = "Geschlecht") 
```
...
```{r Dichte-Diagramm}
ggplot(daten_deutschland, aes(trstprl, fill = gndr)) + 
  geom_density(alpha = 0.5) + 
  ggtitle("Vertrauen in den Bundestag nach Geschlecht") + 
  xlab("Vetrauen (0-10)") + ylab("Anteil der Respondenten") + labs(fill = "Geschlecht") 
```
...
vertrauen_nach_partei <- daten_deutschland %>% 
  rename(Partei = prtvede1) %>%  # prtvede1 = Party voted for in last national election 
  group_by(Partei) %>% 
  summarise(Vertrauenswürdigkeit = mean(trstplc, na.rm = T))  # ppltrst = Most people can be trusted or you can't be too careful

ggplot(vertrauen_nach_partei, aes(Partei, Vertrauenswürdigkeit, group = 1)) + 
  geom_line() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  ggtitle("Vertrauen in andere Menschen nach Partei")
```
...
ggplot(daten_auswahl, aes(agea, netustm)) +
  geom_point()
```
...
ggplot(daten_auswahl, aes(polintr, nwspol)) +
...
ggplot(daten_auswahl, aes(polintr, nwspol, color = cntry, shape = gndr)) +
...
internetnutzung_aggregiert <- internetnutzung %>% 
  group_by(Altersgruppe, Geschlecht, Land) %>% 
  summarise(Internetnutzung = median(Internetnutzung))

ggplot(internetnutzung_aggregiert, aes(Land, Internetnutzung, fill = Geschlecht)) +
...
```{r Ein Boxplot}
ggplot(boxplotdaten, aes(Glücklich, Internetnutzung)) +
  geom_boxplot()
```
...
ggplot(boxplotdaten, aes(Glücklich, Internetnutzung, fill = Gewählt)) +
  geom_violin() + 
  geom_jitter(aes(Glücklich, Internetnutzung), position = position_jitter(width = 0.4, height = 0), 
