...
fit =  fa(allbus_vertrauen, factors = 1, fm = "ml", rotation = "varimax") # <1>

print(fit, digits = 2, cutoff = .3) # <2>
```

1. Mit der `fa` Funktion führen wir eine Faktorenanalyse durch. Hierfür wählen wir die Anzahl an Faktoren mit `factors` aus, legen ein Rotation mit `rotate` sowie eine Methode der Schätzung mittels `fm` fest
2. Anzeigen der Ergebnisse mit 2 Nachkommastellen und dem Ausblenden von Faktorladungen die kleiner als 0.3 sind
